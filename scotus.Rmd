...
scdbv %>%
  ...
  ggplot(aes(term, n_pct)) + geom_line() + scale_y_continuous(labels=scales::percent, limits=c(0,NA)) +labs(title= "Percent of Time Scalia was in the Majority", x = "Term", y= "Percent of Total Decisions")
...
ggplot(scdbv3, aes(term, n_pct, color=type))+geom_line()+scale_y_continuous(labels = scales::percent, limits=c(0,NA))+ labs(titles = "Percent of Time Scalia was in the Majority", x="Term", y = "Percent of Total Decisions")
...
scdbv %>%
  ...
  group_by(term)%>%
 count(term, month) %>%
  ggplot(aes(month, n, group= term)) + geom_boxplot() +coord_flip()
...
scdbv6%>%
  ...
  mutate(percent = n / sum(n))%>%
  filter(decisionDirection == 1)  %>%
  ggplot(aes(x = term, y = percent))+ geom_line()+ facet_wrap(~chief)
